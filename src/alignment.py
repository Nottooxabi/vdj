...
def define_cdr2(seq, v, j):

    for a in pairwise2.align.globalms(seq, v, match=2, mismatch=-1, open=-10, extend=-0.1,
                                     one_alignment_only=True):
        print(format_alignment(*a))

    for b in pairwise2.align.globalms(seq, j, match=2, mismatch=-1, open=-10, extend=-0.1,
                                     one_alignment_only=True):
